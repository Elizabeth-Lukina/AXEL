...
def save_city(message):
    ...
    bot.send_message(message.chat.id,
                     f"Отлично, я запомнил твой город: {city}.\nТеперь каждый день ты будешь получать сводку по погоде, выбранного тобой города.",
...
def handle_subscription_choice(message):
    ...
    if answer == "да":
        ...
        bot.send_message(chat_id, "Во сколько присылать утреннюю сводку? Введи время в формате ЧЧ:ММ")
    else:
        bot.send_message(chat_id, "Хорошо! Ты можешь включить рассылку позже через меню.",
                         reply_markup=get_main_menu())
...
def save_send_time(message):
    try:
        ...
        if 0 <= hour < 24 and 0 <= minute < 60:
            ...
            bot.send_message(message.chat.id,
                             f"Готово! Сводка теперь будет приходить в {hour:02}:{minute:02}.",
                             reply_markup=get_main_menu())
        else:
            raise ValueError
    except:
